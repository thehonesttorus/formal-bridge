...
} from '\var\task\Desktop\Insolvency\formal-bridge/.netlify/dist/run/handlers/request-context.cjs'
import { getTracer, withActiveSpan } from '\var\task\Desktop\Insolvency\formal-bridge/.netlify/dist/run/handlers/tracer.cjs'

process.chdir('\var\task\Desktop\Insolvency\formal-bridge')
  ...
  const handlerResponse = await runWithRequestContext(requestContext, () => {
    return withActiveSpan(tracer, 'Next.js Server Handler', async (span) => {
      span?.setAttributes({
        ...
        monorepo: true,
        cwd: '\var\task\Desktop\Insolvency\formal-bridge',
      })
      if (!cachedHandler) {
        const { default: handler } = await import('\var\task\Desktop\Insolvency\formal-bridge\.netlify\dist\run\handlers\server.js')
